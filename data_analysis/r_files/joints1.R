...
single_boxplot <- function(phrase){
  ...
  write_csv(anova_table, "rANOVA_results.csv")
  ...
  write_csv(pwc, "t_test_results.csv")
  print(pwc)
  ...
  return(qplot(x=condition, y=total, data = new_target[complete.cases(new_target),], geom="boxplot", fill=condition)+ geom_boxplot()+
           ...
           scale_y_continuous(breaks=1:7, labels=c('Min','','','','','', 'Max'), limits=c(1,7))
  ) 
...
name <- single_boxplot('_1')+ guides(fill="none")
ggsave('PLOT2.png', units = 'in', width=5,height=5)
