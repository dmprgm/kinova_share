...
folder_path <- paste0("C:/Users/Student/Documents/TestResultsv002/", var)
...
single_boxplot <- function(var){
  ...
  # Plot formatting
  return(
    ggplot(data = combined_data[complete.cases(combined_data),], aes(x = condition, y = total, fill = phrase)) + 
      ...
      geom_signif(
        comparisons = list(c("A", "B"), c("C", "D"), c("E", "F"), c("G", "H")),
        scale_fill_npg(),
