...
def compile_joint_states(bag_file, position_threshold=0.01, velocity_threshold=0.01): #adjust these thresholds if needed?
    ...
    for topic, msg, t in bag.read_messages(topics=['/joint_states']):
        ...
        current_efforts = np.array(msg.effort)  # Track efforts
        ...
        if movement_started:
            joint_states.append((t.to_sec(), current_positions, current_velocities, current_efforts))

        # Update previous positions, velocities, and efforts
        prev_positions = current_positions
        prev_velocities = current_velocities
        prev_efforts = current_efforts
...
def compute_attributes(joint_states):
    ...
    efforts = np.array([state[3] for state in joint_states])
...
def save_to_csv(filename, times, positions, velocities, efforts):
    with open(filename, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['Time', 'Position', 'Velocity', 'Effort'])
        for t, pos, vel, eff in zip(times, positions, velocities, efforts):
            csvwriter.writerow([t, pos, vel, eff])
        ...
        if os.path.isdir(subdir_path):  # Only process directories like P0, P1, etc.
            for file in os.listdir(subdir_path):
                if file.endswith('.bag'):
                    ...
                    times, positions, velocities, efforts = compute_attributes(joint_states)
                    save_to_csv(output_csv, times, positions, velocities, efforts)
                    
                    print(f"Data saved to {output_csv}")


if __name__ == "__main__":
    study_bags_directory = '/home/sharer/hw_test_ws/src/rec_rep/cleaned_study_bags'
